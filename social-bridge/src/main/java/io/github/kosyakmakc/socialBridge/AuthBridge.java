...
public class AuthBridge implements IAuthBridge {
    ...
    private AuthBridge(IMinecraftPlatform mcPlatform) throws SQLException, IOException {
        ...
        socialPlatforms = List.of(new ISocialPlatform[0]); //ServiceLoader.load(ISocialPlatform.class, ISocialPlatform.class.getClassLoader()).stream().map(ServiceLoader.Provider::get).toList();
